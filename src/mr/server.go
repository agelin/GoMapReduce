...
func RunServer(inputdir string, output io.Writer, quitServer chan bool) {
	...
	for k := 1; k < len(NodesMap); k++ {
		...
		c, err := RetryDial("tcp", v, DialTimeOut*time.Second)
		if err != nil {
			log.Fatal(err)
		}
	...
	for _, f := range files {
		if !f.IsDir() {
			...
			for mr, v := range md {
				...
				iwip := NodesMap[mr]
				if iwc, err = RetryDial("tcp", iwip, DialTimeOut*time.Second); err != nil {
					log.Fatal(err)
				}
			...
			case ReduceWorkersMSG:
				...
				if len(workingMappers) == 0 {
					...
					for k, v := range inReducer {
						...
						ip := NodesMap[k]
						var rc net.Conn
						if rc, err = RetryDial("tcp", ip, DialTimeOut*time.Second); err != nil {
							log.Fatal(err)
						}
						var msg ActionMessage
						msg.Msg = ReduceMSG
